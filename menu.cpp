...
void create_main_menu_panel(Engine* engine, PagesControl* menu) {
    ...
    std::string server = engine->getPaths()->getWorldsFolder().string() + "/Server 1";
    if (fs::is_directory(server)) {
...
void create_online_panel(Engine* engine, PagesControl* menu) {
    auto panel = create_page(engine, "online", 400, 0.0f, 5);  //menu initialize


    //here starts Username Label

    panel->add(std::make_shared<Label>(langs::get(L"Username:", L"username")));
    auto usernameInput = std::make_shared<TextBox>(L"Notch", vec4(6.0f));



    panel->add(usernameInput);

    //here end Username label

    //server ip label starts

    panel->add(std::make_shared<Label>(langs::get(L"Server IP:", L"serverip")));
    auto ipInput = std::make_shared<TextBox>(L"127.0.0.1", vec4(6.0f));


    panel->add(ipInput);

    panel->add(std::make_shared<Label>(langs::get(L"Server Port:", L"serverport")));
    auto portInput = std::make_shared<TextBox>(L"8080", vec4(6.0f));


    panel->add(portInput);

    auto checkbox = new FullCheckBox(langs::get(L"Private", L"private"), vec2(400, 32));

    panel->add(checkbox);



    //here end server ip label

    //here starts a join button


    panel->add(create_button( L"Join", vec4(10), vec4(1, 20, 1, 1),
                              [=](GUI*){



                                  std::wstring user = usernameInput->text();
                                  std::wstring ip = ipInput->text();
                                  std::wstring wport = portInput->text();
                                  std::cout << "Username and Ip sync success!" << std::endl;





                                  tryConnectAsClient(ip, user, wport);


                                  std::string nameutf8 = "Server 1";
                                  EnginePaths* paths = engine->getPaths();

                                  uint64_t seed = 0;
                                  std::cout << "world seed: " << seed << std::endl;
                                  std::cout << "Server seted up!" << std::endl;
                                  ...
                                  Level* level = World::create(
                                          ...
                                          engine->getContentPacks());
                                  engine->setScreen(std::make_shared<LevelScreen>(engine, level));
                                  std::cout << "Joined to server success!" << std::endl;
                              }));
    panel->add(create_button( L"Join As Server", vec4(10), vec4(1, 20, 1, 1),
                              [=](GUI*){
                                  ...
                                  tryConnectAsServer(ip, user, wport);
    ...
    //here start Join as server


    //here end Join as server
    panel->add(guiutil::backButton(menu)); //Back button
}
...
void menus::create_menus(Engine* engine, PagesControl* menu) {
    ...
    create_online_panel(engine, menu);
}
